  ...
  function Model(props, converters, diffFn)
  {
    ...
    _.forEach(converters, function(value, key) {
      ...
      self["to" + fName] = function() {
        if (_.isObject(converters) && converters.hasOwnProperty(key)) {
          if (_.isFunction(converters[key].toFn))
            return converters[key].toFn(self);
        }

        throw new Error("Cannot convert it to ", key);
      };
    });
